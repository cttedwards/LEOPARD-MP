...
for (i in 1:nreps) {
  ...
  for (y in 2:nyr.proj) {
    ...
    removals <- list(trophy         = list(rate = harvest.rate, preference = selectivity), 
                     ...
                     aging_error    = list(rate = 0.05, preference = only.males))
...
par(bg = NA) 
#par(bg = "white") 
cairo_pdf(file = '/Users/RossTyzackPitman/Documents/OneDrive/Data/GitHub/Databases/PhD_Chapter3/MSE_Paper/figures/Population.Size.Model.24.pdf', 
          width = 8, height = 5)
boxplot(x.tot[,11:nyr.proj],
        ...
        ylim = c(0,10000),
        outline = FALSE)
title("Hunting = 50% males \u2265 7 yrs; Aging error = 9%;\n Illegal killing = 1%; Problem animal control = 1%", line = -3)
...
par(bg = NA) 
#par(bg = "white") 
pdf(file = '/Users/RossTyzackPitman/Documents/OneDrive/Data/GitHub/Databases/PhD_Chapter3/MSE_Paper/figures/Eigen.Value.Model.24.pdf', 
    width = 8, height = 5)
