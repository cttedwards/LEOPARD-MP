model{
  ...
  # censoring mechanism  
  censored[i] ~ dinterval(la.time[i],lctime[i])
  
  # sampling model
  la.time[i] ~ dlogis(mu[i],tau)
  
  # linear predictor
  mu[i] <- betas[1:n.covs] %*% COVS[i,1:n.covs] + habitat[hab[i]] + taxonomy[tax[i]]
  
  CS[i] <- -log(1-plogis(log(ctime[i]),mu[i],tau))
