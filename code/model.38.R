...
for (i in 1:nreps) {
  ...
  for (y in 2:nyr.proj) {
    ...
    removals <- list(trophy         = list(rate = harvest.rate, preference = selectivity), 
                     ...
                     aging_error    = list(rate = 0.03, preference = only.males))
...
par(bg = NA) 
#par(bg = "white") 
cairo_pdf(file = '/Users/RossTyzackPitman/Documents/OneDrive/Data/GitHub/Databases/PhD_Chapter3/MSE_Paper/figures/Population.Size.Model.38.pdf', 
          width = 8, height = 5)
boxplot(x.tot,
        ...
        ylim = c(0,2000),
        outline = FALSE)
title("Hunting = 100% males \u2265 7 yrs; Aging error = 3%", line = -2)
axis(side = 1, at = 1:nyr.proj)
dev.off()

par(bg = NA) 
#par(bg = "white") 
pdf(file = '/Users/RossTyzackPitman/Documents/OneDrive/Data/GitHub/Databases/PhD_Chapter3/MSE_Paper/figures/Eigen.Value.Model.38.pdf', 
    width = 8, height = 5)
